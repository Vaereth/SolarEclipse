    ...
    return (
      <button onClick={() => onOpen(d)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
          <span style={{ fontFamily: "'Silkscreen', monospace", fontSize: 12, color: '#7a6c4a' }}>No.{d.dex}</span>
          {formCount > 0 && <span style={{ fontFamily: "'Silkscreen', monospace", fontSize: 8, color: '#ffb347' }}>+{formCount} {formCount === 1 ? 'FORM' : 'FORMS'}</span>}
        </div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: 10, borderTop: '1px solid #1c1609' }}>
          <span style={{ fontFamily: "'Silkscreen', monospace", fontSize: 8, color: '#7a6c4a' }}>BST</span>
    ...
    return (
      <div>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8, paddingTop: 10, borderTop: '1px solid #2a2110' }}>
          <span style={{ fontFamily: "'Silkscreen', monospace", fontSize: 9, color: '#b8a489' }}>TOTAL</span>
        ...
        <div style={{ marginTop: 14 }}>
          <div style={{ fontFamily: "'Silkscreen', monospace", fontSize: 9, color: '#8a7d63', marginBottom: 6, letterSpacing: 0.5 }}>ABILITIES</div>
        ...
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 720, background: 'linear-gradient(160deg, #14100a, #0a0805)', border: `1px solid ${accent}44`, borderRadius: 18, boxShadow: `0 0 60px ${accent}22, 0 30px 80px #000a`, overflow: 'hidden' }}>
          ...
          <div style={{ padding: 22, display: 'grid', gridTemplateColumns: '180px 1fr', gap: 24, alignItems: 'start' }} className="se-detail-grid">
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ fontFamily: "'Pixelify Sans', sans-serif", fontWeight: 700, fontSize: 28, color: '#fff', textAlign: 'center', lineHeight: 1 }}>{cur.name}</div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 16 }}>
                  {variants.map((v, i) => (
                    <button key={i} onClick={() => setVi(i)} style={{
                      cursor: 'pointer', padding: '5px 11px', borderRadius: 8, fontSize: 12, fontFamily: "'Space Grotesk', sans-serif",
      ...
      <div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16, marginBottom: 16, flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            {filters.length > 0 && <button onClick={() => setFilters([])} style={{ cursor: 'pointer', padding: '4px 10px', borderRadius: 999, fontSize: 11, color: '#ff8f6f', background: 'transparent', border: '1px solid #5e3020', fontFamily: "'Space Grotesk', sans-serif" }}>clear ×</button>}
          </div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <span style={{ fontFamily: "'Silkscreen', monospace", fontSize: 9, color: '#7a6c4a' }}>SORT</span>
